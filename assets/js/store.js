    ...
    const teamLeaderInput = document.getElementById('teamLeaderInput');
    const managerEmployeeInput = document.getElementById('managerEmployeeInput');
    ...
    function refreshEditorGroupOptions(selectedValue = '') {
      clearSelectOptions(groupInput, '담당을 선택하세요');
      appendSelectOptions(groupInput, allManagers, 'group_name');
      groupInput.value = selectedValue && allManagers.some((item) => String(item.id) === String(selectedValue)) ? String(selectedValue) : '';
    }

    function refreshEditorTeamLeaderOptions(selectedValue = '') {
      const selectedGroupId = Number(groupInput.value || 0);
      if (!selectedGroupId) {
        clearSelectOptions(teamLeaderInput, '담당을 먼저 선택하세요');
        teamLeaderInput.disabled = true;
        teamLeaderInput.value = '';
        return;
      }

      const candidates = allTeamLeaders.filter((item) => Number(item.group_id) === selectedGroupId);
      clearSelectOptions(teamLeaderInput, candidates.length ? '팀장을 선택하세요' : '선택 가능한 팀장이 없습니다');
      candidates.forEach((leader) => {
        const option = document.createElement('option');
        option.value = String(leader.employee_no || '');
        option.textContent = leader.team_leader_name;
        teamLeaderInput.appendChild(option);
      });
      teamLeaderInput.disabled = candidates.length === 0;
      teamLeaderInput.value = selectedValue && candidates.some((item) => String(item.employee_no) === String(selectedValue)) ? String(selectedValue) : '';
    }
    ...
    function setEditorMode(mode, store = null) {
      if (mode === 'create') {
        ...
        managerEmployeeInput.value = '';
        refreshEditorGroupOptions();
        refreshEditorTeamLeaderOptions();
      } else {
        ...
        managerEmployeeInput.value = normalizeText(store.manager_employee_no);
        refreshEditorGroupOptions(store.group_id);
        refreshEditorTeamLeaderOptions(store.team_leader_employee_no);
      }
    ...
    async function handleSave(event) {
      ...
      const groupId = groupInput.value ? Number(groupInput.value) : null;
      const teamLeaderEmployeeNo = normalizeText(teamLeaderInput.value) || null;
      const managerEmployeeNo = normalizeText(managerEmployeeInput.value) || null;
      ...
      try {
        ...
        const payload = { storeName, isActive, groupId, teamLeaderEmployeeNo, managerEmployeeNo };
