...
function getEmployeeRelationValue(employee, keys) {
  for (const key of keys) {
    if (employee && Object.prototype.hasOwnProperty.call(employee, key) && employee[key] !== null && employee[key] !== undefined) {
      return employee[key];
    }
  }
  return '';
...
async function loadEmployeeDetail(employeeNo) {
  ...
  const managerValue = getEmployeeRelationValue(employee, ['current_manager_id', 'group_id']);
  const teamLeaderValue = getEmployeeRelationValue(employee, ['current_team_leader_id', 'team_leader_id']);
  const storeValue = getEmployeeRelationValue(employee, ['current_store_id', 'store_id']);

  document.getElementById('current_manager_id').value = managerValue ?? '';
  ...
  if (teamLeaderValue !== '') {
    ...
    teamLeaderSelect.value = String(teamLeaderValue);
  }
...
async function updateEmployee(formData) {
  ...
  const selectedTeamLeader = allTeamLeaders.find((item) => String(item.id) === String(formData.current_team_leader_id));
  const selectedStoreId = toNullableNumber(formData.current_store_id);
  const selectedManagerId = toNullableNumber(formData.current_manager_id);

  if (loadedEmployee && Object.prototype.hasOwnProperty.call(loadedEmployee, 'current_store_id')) {
    employeePayload.current_store_id = selectedStoreId;
  } else if (loadedEmployee && Object.prototype.hasOwnProperty.call(loadedEmployee, 'store_id')) {
    employeePayload.store_id = selectedStoreId;
  }

  if (loadedEmployee && Object.prototype.hasOwnProperty.call(loadedEmployee, 'current_manager_id')) {
    employeePayload.current_manager_id = selectedManagerId;
  } else if (loadedEmployee && Object.prototype.hasOwnProperty.call(loadedEmployee, 'group_id')) {
    employeePayload.group_id = selectedManagerId;
  }

  if (loadedEmployee && Object.prototype.hasOwnProperty.call(loadedEmployee, 'current_team_leader_id')) {
    employeePayload.current_team_leader_id = toNullableNumber(formData.current_team_leader_id);
  } else if (loadedEmployee && Object.prototype.hasOwnProperty.call(loadedEmployee, 'team_leader_id')) {
    employeePayload.team_leader_id = toNullableNumber(formData.current_team_leader_id);
  } else if (loadedEmployee && Object.prototype.hasOwnProperty.call(loadedEmployee, 'team_leader_employee_no')) {
    employeePayload.team_leader_employee_no = selectedTeamLeader?.employee_no || null;
  }
